...
export const metrics = async (req, res) => {
  try {
    ...
    // Fetch query params
    const { period = "daily", branch = "all" } = req.query;
    ...
    switch (period) {
      case "monthly":
        chartData = await query.select("*").orderBy("year", "desc").orderBy("month", "desc");
        break;
      case "yearly":
        chartData = await query.select("*").orderBy("year", "desc");
        break;
      default: // daily
        chartData = await query.select("*").orderBy("sale_date", "desc");
    }
    ...
    switch (period) {
      ...
      case "monthly":
        ...
        const currentMonthData = filledChartData.find((item) => item.period === currentMonthLabel);
      ...
      case "yearly":
        ...
        const currentYearData = filledChartData.find((item) => item.period === currentYearLabel);
    ...
    let lowStockItems = await knex("count_low_stock").select("low_stock").first();
